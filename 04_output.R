...
for (i in 1:num) {
  ...
  GBAR<-data.frame(lapply(GBlistA, function(x) rank(x, ties.method='random')))
  ...
  GBD<-data.frame(lapply(GBlistD, function(x) round((x)/GBlistD$Area,4)))
  ...
  ThreatZ<-
    cbind(data.frame(ThreatZone[ , (names(ThreatZone) %in% Strata)], GBA, GBAR, GBL, GBLR, GBC, GBD, GBDR)) %>%
...
for (i in 1:num) {
  ...
  ThreatLZR[[StratName]]<-data.frame(GBPU_Name=ThreatLR[[StrataL[i]]]$POPULATION_NAME,
                                     ...
                                     Residential_1Rank=ThreatLR[[StrataL[i]]]$Residential_1Rank,
                                     ...
                                     Transport_4.2Rank=ThreatLR[[StrataL[i]]]$Transport_4.2Rank,
...
ThreatI<-data.frame(GBPU_Name=ThreatLR[['GBPUr']]$POPULATION_NAME,
                    ...
                    Residential_1Rank=ThreatLR[['GBPUr_LFormFlat']]$Residential_1Rank,
                    ...
                    Transport_4.2Rank=ThreatLR[['GBPUr_NonHab']]$Transport_4.2Rank,
...
ThreatSummary <- function(DF, Threat_A, Threat_NS) {
  DF %>%
    ...
    mutate(MT = as.integer(DF[[Threat_A]])) %>%
    ...
    group_by(Decile) %>%
    dplyr::summarise(count=n(), MThreat=round(mean(MT),4), Unknown=sum(N_Unknown), Negilible=sum(N_Negligible), Low=sum(N_Low), Med=sum(N_Med))
...
ThreatSummary <- function(DF, Threat_A, Threat_NS) {
  ...
  DF<-ThreatI  
  Threat_A <- c('HumanIntrusion_6')
  Threat_NS <- c('HumanIntrusion')
  ...
  DF %>%
    ...
    mutate(N_Med = as.integer(DF[[Threat_NS]] == 'Med')) %>% 
    group_by(Decile) %>%
    dplyr::summarise(count=n(), MThreat=round(mean(MT),2), Unknown=sum(N_Unknown), Negilible=sum(N_Negligible), Low=sum(N_Low), Med=sum(N_Med))
  #Adplyr::summarise(MT=mean(), count=n(), Unknown=sum(N_Unknown), Negilible=sum(N_Negligible), Low=sum(N_Low), Med=sum(N_Med))
